# THIS SCRIPT WAS ONLY FOR COLLECTING & SCRAPING A SPECIFIC CONTACT
...
SEARCH_TERMS = ["church", "hotel", "conference center", "mall", "headquarters", "school"]
CONTACT_PROSPECTS = utils.SYSTEM_MONGO.get_collection('terminal.payvance_airkitchen')


def search_location():

    location = utils.DB_REGIONS.find_one({
    ...
    for term in SEARCH_TERMS:
        if 'searched_terms' in location and term in location['searched_terms']:
            continue
        staged_finder(center, viewport, term, course_zoom=15, log_name=location['name'])
...
def stage_caller(run_identifier, term, stage, batch_size, zoom, log):
    ...
    while True:
        ...
        try:
            results and CONTACT_PROSPECTS.insert_many(results, ordered=False)
