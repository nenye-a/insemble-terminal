...
sys.path.extend(['/Users/colin/Desktop/insemble-terminal/backend/data'])
import preprocess
import utils
import google
import requests
...
def personal_reports(csv_filename):
    ...
    for i in contact_df.index:
        ...
        type = contact_df["Type"][i]
        company = contact_df["Company"][i]
        address = contact_df["Address"][i]
        city = contact_df["City"][i]

        print("Servicing {}. Finding locations for {}, a {} near {} {}".format(name, company, type, address, city))
...
def logic_handler(company, address, city, type):
    ...
    if 'retailer' in type:
        ...
        count_documents = utils.DB_TERMINAL_PLACES.count_documents({"name": processed_brand})
        if count_documents:
        ...
        if not matches:
            ...
            if subsidiaries:
                for sub in subsidiaries:
                    ...
                    count_documents = utils.DB_TERMINAL_PLACES.count_documents({"name": processed_sub})
                    if count_documents:
                        matches = utils.DB_TERMINAL_PLACES.find(
                            {"name": processed_sub,
        ...
        if not matches:
            ...
            count_documents = utils.DB_TERMINAL_PLACES.count_documents({"name": processed_default})
            if count_documents:
                matches = utils.DB_TERMINAL_PLACES.find(
    ...
    # if type landlord
    if ('owner' or 'hospitality industry' or 'shopping center management') in type:
        NUM_CENTER_COMPS = 4
    ...
    # if type broker
    if ('real estate services' or 'retail broker' or 'tenant services') in type:
        return None

    # if type municipality
    if 'public sector' in type:
        return None

    # if type scholar
    if ('academic institution' or 'student') in type:
        return None

    # if type people researcher
    if ('Consumer/Market Research'.lower() or 'Financial/Investment Services'.lower() or 'Parking and Traffic'.lower()) in type:
        return None
    ...
    if ('advertising/marketing/pr' or 'architecture/design/engineering' or 'Building Materials/Structural'.lower() or
        ...
        'publications/publishers' or 'trade association' or 'utilities/telecommunication') in type:
        return None
...
def find_nearby_competitor_with_activity(brand, category, location):
    ...
    most_likely_competitors = sorted(Counter(same_cat_brands).items(), key=lambda x:x[1], reverse=True) # TODO: make place matching better for name variations
